...
# with modifications originally by Jen Mandel then by Marisa Blake Szubryt (2026-08-27)
...
# Run analysis with runslow=TRUE then set runslow=FALSE for 'real' run
runslow <- TRUE
resfn <- 'Psychotria_DEC_M0_unconstrained_v1.Rdata'
if (runslow) {
  res = bears_optim_run(BioGeoBEARS_run_object); print(res)  
  save(res, file=resfn); resDEC = res
} else {
  load(resfn); resDEC = res
}
...
if (runslow) {
  ...
  save(res, file=resfn); resDECj = res
} else {
  load(resfn); resDECj = res
}
...
runslow = TRUE
if (runslow) {
  res = bears_optim_run(BioGeoBEARS_run_object); res; save(res, file=resfn)
  resDIVALIKEj = res
} else {
  load(resfn)
  resDIVALIKEj = res
